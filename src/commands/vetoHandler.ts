...
async function handleImagesStep(ctx: any, session: any) {
  ...
  if (ctx.message.photo) {
    ...
    await ctx.reply(
      `📷 Image ${updatedImages.length}/5 added.\n\n<i>Send more images or proceed to review. You can also send multiple images at once.</i>`,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: '✅ Done with Images', callback_data: 'veto_done_images' }]
          ]
        }
      }
    );
    ...
    if (fileId) {
      ...
      if (currentImages.length < 5) {
        ...
        await ctx.reply(
          `📷 Images received (${updatedImages.length}/5).\n\n<i>Processing media group...</i>`,
          {
            reply_markup: {
              inline_keyboard: [
                [{ text: '✅ Done with Images', callback_data: 'veto_done_images' }]
    ...
    'What would you like to modify?',
    {
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '🔙 Back to Review', callback_data: 'veto_back_review' }]
        ]
      }
    }
  );
